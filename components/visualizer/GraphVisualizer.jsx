...
export default function GraphVisualizer() {
  ...
  const COLORS = ['var(--gx-accent)','var(--gx-warning)','var(--gx-success)','var(--gx-warning)','var(--gx-danger)','var(--gx-info)'];
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
        {[
          { label: 'Vertices', value: nodes.length, color: 'var(--gx-accent)' },
          { label: 'Edges', value: edges.length, color: 'var(--gx-warning)' },
          { label: 'Type', value: directed ? 'Directed' : 'Undirected', color: 'var(--gx-warning)' },
          { label: 'Selected Edge', value: selectedEdge !== null ? `${edges.find(e=>e.id===selectedEdge)?.u}→${edges.find(e=>e.id===selectedEdge)?.v}` : '—', color: 'var(--gx-success)' },
        ].map(s => (
      ...
      {message && (
        <div style={{ background: 'var(--gx-accent-soft)', border: '1px solid var(--gx-border)', borderRadius: 8, padding: '8px 14px', fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--gx-accent)' }}>▶ {message}</div>
      )}
      ...
      <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
              ...
              return (
                <g key={e.id} onClick={() => { setSelectedEdge(isSel ? null : e.id); setMessage(isSel ? 'Deselected.' : `Edge ${e.u}→${e.v}, weight=${e.w}`); }} style={{ cursor: 'pointer' }}>
                  <line x1={nu.x} y1={nu.y} x2={directed ? ex : nv.x} y2={directed ? ey : nv.y}
                    stroke={isSel ? 'var(--gx-accent)' : 'var(--gx-accent-border)'} strokeWidth={isSel ? 2.5 : 1.5}
                    markerEnd={directed ? (isSel ? 'url(#arrow-sel)' : 'url(#arrow)') : undefined} />
                  <rect x={mid.x - 12} y={mid.y - 10} width={24} height={18} rx={4}
                    fill={isSel ? 'var(--gx-accent-soft)' : 'var(--gx-surface)'} stroke={isSel ? 'var(--gx-accent)' : 'var(--gx-border)'} strokeWidth={1} />
                  <text x={mid.x} y={mid.y + 4} textAnchor="middle"
                    style={{ fontFamily: 'var(--font-mono)', fontSize: 11, fontWeight: 700, fill: isSel ? 'var(--gx-accent)' : 'var(--gx-text-muted)' }}>{e.w}</text>
                </g>
              );
            })}
            {nodes.map(n => (
              <g key={n.id}>
                <circle cx={n.x} cy={n.y} r={22} fill="var(--gx-text-inverse)" stroke={COLORS[n.id]} strokeWidth={2} />
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 12, minWidth: 200 }}>
          <div style={{ background: 'var(--gx-surface)', border: '1px solid var(--gx-border)', borderRadius: 10, padding: 12 }}>
            <div style={{ fontFamily: 'var(--font-body)', fontSize: 10, color: 'var(--gx-text-muted)', letterSpacing: 1, marginBottom: 8 }}>ADJ LIST</div>
            {adjList.map((nbrs, i) => (
              <div key={i} style={{ display: 'flex', gap: 6, alignItems: 'center', marginBottom: 3 }}>
                ...
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--gx-text-muted)' }}>{nbrs.map(n=>`${n.to}(${n.w})`).join(', ') || '—'}</span>
              </div>
            ))}
          </div>

          <div style={{ background: 'var(--gx-surface)', border: '1px solid var(--gx-border)', borderRadius: 10, padding: 12 }}>
            <div style={{ fontFamily: 'var(--font-body)', fontSize: 10, color: 'var(--gx-text-muted)', letterSpacing: 1, marginBottom: 8 }}>ADJ MATRIX</div>
              ...
              <table style={{ borderCollapse: 'collapse' }}>
                <tbody>
                  {adjMatrix.map((row, i) => (
                    <tr key={i}>
                      {row.map((v, j) => (
                        <td key={j} style={{ width: 28, height: 24, textAlign: 'center', fontFamily: 'var(--font-mono)', fontSize: 10, color: v !== '∞' && v !== 0 ? 'var(--gx-accent)' : i === j ? 'var(--gx-warning)' : 'var(--gx-text-subtle)', border: '1px solid var(--gx-border)', background: 'var(--gx-surface-2)' }}>{v}</td>
                      ))}
                    </tr>
                  ))}
                </tbody>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
        <button onClick={() => setDirected(d => !d)} style={btn(directed ? 'var(--gx-warning)' : 'var(--gx-warning)')}>{directed ? 'Directed ✓' : 'Undirected'}</button>
        <div style={{ display:'flex', gap:4, alignItems:'center' }}>
          <input type="number" value={addU} onChange={e=>setAddU(e.target.value)} placeholder="U" style={inputStyle} />
          <span style={{ color:'var(--gx-text-muted)', fontFamily:'var(--font-mono)' }}>→</span>
          ...
          <button onClick={addEdge} style={btn('var(--gx-success)')}>Add Edge</button>
        </div>
        {selectedEdge !== null && <button onClick={() => removeEdge(selectedEdge)} style={btn('var(--gx-danger)')}>Remove Selected</button>}
        <button onClick={reset} style={btn('var(--gx-text-muted)')}>↺ Reset</button>
      </div>
...
const inputStyle = { background:'var(--gx-accent-soft)', border:'1px solid var(--gx-accent-border)', borderRadius:8, padding:'6px 10px', color:'var(--gx-text)', fontFamily:'var(--font-mono)', fontSize:12, outline:'none', width:56 };
