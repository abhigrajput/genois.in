...
export const CURATED_VIDEOS = [
  ...
  {
    slug: 'dsa-strings',
    // Was Part 2 of this series, which dropped a first-time learner into the
    // middle of it. This is Part 1 of the same series, same teacher.
    id: 'MOSjYaVymcU',
  ...
  },
  {
    ...
    // Was N-Queens — a LeetCode Hard, lesson 14 of a series — as a student's
    // first contact with backtracking. This is an actual introduction, and it
    // completes the Abdul Bari paradigm set already here: 2 Divide And Conquer
    // (dsa-divide-conquer), 3 Greedy Method (dsa-greedy), 6 Backtracking.
    id: 'DKCbsiDBN6c',
  ...
  {
    slug: 'cs-net-http',
    // Was a 56-second Short standing in for HTTP — no methods, status codes,
    // headers or HTTPS. This is a full lecture from the channel that already
    // carries the OS and DBMS entries in this file.
    id: 'pnoWCK82apU',
  ...
  },
  {
    ...
    // Was a different teacher and series wedged between cpp-variables
    // (Lecture 2) and cpp-functions (Lecture 5), and the least-watched video in
    // the file. This is literally the missing Lecture 3 of that same series.
    // It teaches loops as well as conditionals, which is why cpp-loops below
    // is Lecture 4 (pattern drills) rather than another loops explainer.
    id: 'qR9U6bKxJ7g',
  ...
  {
    slug: 'cpp-loops',
    // Was a third teacher across four consecutive C++ lessons. This series has
    // no standalone loops lecture — Lecture 3 (cpp-conditionals) teaches them —
    // so loops day is Lecture 4, which is nothing but nested-loop practice.
    // The four cpp topics are now Lectures 2, 3, 4 and 5 of one series, in
    // order: meet loops on the conditionals day, drill them on this one.
    id: 'rga_q2N7vU8',
  ...
  },
  {
    ...
    // Was a 76-minute video for the simplest algorithm in the file. 13 minutes,
    // and it sits in the C++ track the rest of the map teaches in.
    id: '13ocRMSJy5M',
    title: '8.2 Searching in Arrays | Linear and Binary Search | C++ Placement Course |',
    channel: 'Apna College',
 ...
 */
export function embedUrl(video, { autoplay = false, jsapi = false, origin = null } = {}) {
  const id = typeof video === 'string' ? video : video?.id;
  if (!id) return null;
  const params = new URLSearchParams({ rel: '0', modestbranding: '1', playsinline: '1' });
  if (autoplay) params.set('autoplay', '1');
  // `jsapi` opts the player into its postMessage channel, which is the only way
  // to hear onError (100 = gone, 101/150 = embedding disabled). <VideoPlayer>
  // uses it to swap a dead player for a readable message. `origin` is required
  // by YouTube alongside enablejsapi and must be the page's own origin.
  if (jsapi) {
...
/**
 * Canonical watch page for a video.
 *
 * This is NOT the return of the search link this file exists to kill. It points
 * at one specific, already-curated video, and it is rendered in exactly one
 * place: the fallback shown when the owner has disabled embedding, where the
 * video genuinely is watchable on YouTube and nowhere else. A student who has
 * hit that card has no in-app option left, so sending them off-site beats
 * showing them a dead rectangle. It is never rendered on the happy path, and
 * never for a video that has been deleted (that link would 404 too).
 */
