...
export const roadmapAPI = {
  ...
  // Pattern-based roadmap. GET is only needed by surfaces that don't already
  // load /api/roadmap/daily (which returns the plan inline).
  getPatterns: () => api.get('/api/roadmap/patterns'),
  setProblemSolved: (patternId, problemId, solved) =>
    api.post('/api/roadmap/patterns', { patternId, problemId, solved }),
};
...
export const mentorAPI = {
  // No sendMessage here any more: the POST route it pointed at was deleted
  // along with the ungrounded /mentor prompt templates, and /mentor now
  // redirects. Mentor chat goes through chatbotAPI.sendMessage.
  ...
  // Feature D — the proactive "what to focus on now", derived from the
  // student's real journey signals. No model runs behind it.
  getFocus: () => api.get('/api/mentor/focus'),
};
...
export const analyticsAPI = {
  ...
  // getDailyGraph / getWeeklyGraph removed: both endpoints read the `analytics`
  // table the app never writes, so they always returned empty. They now answer
  // 503 endpoint_retired. Use getInsights instead.
