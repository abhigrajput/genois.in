// BETA_MODE: public beta — every feature is unlocked for all testers.
// Flip to false to re-enable the paid-tier plan gates (the PERMISSIONS map,
// PermissionGate lock screens, and the Player/Performer/Dominator tiers all
// come back to life untouched). Kept as a single switch on purpose.
...
export function getEffectivePlan(userPlan, trialEndsAt, isOnTrial) {
  if (BETA_MODE) return 'dominator'; // public beta: top tier for everyone
...
export function hasPermission(userPlan, feature, trialEndsAt, isOnTrial) {
  if (BETA_MODE) return true; // public beta: nothing is gated
...
export function getUpgradeMessage(feature) {
  const messages = {
    ai_mentor: 'Mentor is available from Performer plan (₹199/month)',
