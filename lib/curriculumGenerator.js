...
Every task today should move the student closer to clearing ${primaryTarget}'s specific interview process. Include at least one task directly relevant to their interview pattern.`
  ...
  const ragQuery = targeting?.targeted ? `${targeting.focus.label} ${phase.focus}` : phase.focus;
  ...
  const contextualPrefix = (systemContext || ragContext || performanceAdaptation || gapInstruction)
    ? `${systemContext}\n${urgencyInstruction}\n${companyInstruction}\n${weaknessInstruction}${performanceAdaptation}${gapInstruction}${ragContext}\n\n`
    : '';
...

{
  ...
  if (!deepseekUrl || !deepseekKey) {
    ...
    for (const model of modelsToTry) {
      try {
        ...
        return withSafeVideo({ ...parsed, focus: focusForCache(targeting), generated_by: `deepseek (${model})` });
      } catch (e) {
  ...
  try {
    ...
    return withSafeVideo({ ...result, focus: focusForCache(targeting), generated_by: 'claude' });
  } catch (e) {
  ...
  // Trees day rather than whatever the day counter happened to land on.
  return withSafeVideo(getStaticFallback(domainSlug, dayNumber, week, phase, isProjectDay, projectThisWeek, targeting));
}

function getStaticFallback(domain, dayNumber, week, phase, isProjectDay, project, targeting = null) {
  const topics = {
