...
export async function generateDayContent(domain, dayNumber, level) {
  console.log('DeepSeek URL:', process.env.DEEPSEEK_BASE_URL);
  console.log('DeepSeek key exists:', !!process.env.DEEPSEEK_API_KEY);
  console.log('Trying DeepSeek with URL:', process.env.DEEPSEEK_BASE_URL + '/chat/completions');
  ...
  const prompt = `You are a senior software engineer creating a structured learning curriculum for Indian engineering students.
...

{
  ...
  for (const model of modelsToTry) {
    try {
      console.log(`Attempting DeepSeek with model: ${model}`);
      ...
      console.log(`DeepSeek successfully generated curriculum using model: ${model}`);
      return { ...parsed, generated_by: `deepseek (${model})` };
    } catch (e) {
      console.log(`DeepSeek failed with model ${model}:`, e.message);
    }
  }
  ...
  try {
    ...
    console.log('Claude also failed, using static fallback');
  }
