...
import { focusForCache } from './roadmapTargeting.js';
import { curatedVideoForDay } from './curatedVideos.js';
import { roadmapTotalDays } from './roadmapCache.js';

// The model is prompted for a "real" YouTube URL but reliably hallucinates video
// ids that embed as "Video unavailable". A model-produced video URL is never
// used and never stored — it is dropped here, at the single point every caller
// of generateDayContent passes through.
//
// In its place the day carries a CURATED video resolved from the topic
// (lib/curatedVideos.js), or nothing at all when the topic is not one we have
// curated. Nothing is invented and no search link is ever produced.
function withSafeVideo(content) {
  if (content && typeof content === 'object') {
    content.video_url = null;
    content.video = curatedVideoForDay(content.topic);
  }
  return content;
}
...
export function buildDayMeta(content) {
  ...
  const meta = {
    ...
    // Which readiness gap this day was planned against (Phase 3). Rides in the
    // existing meta JSONB so gap-aware caching needs no new column; null on
    // every day generated without evidence, which is most of them.
    focus: content.focus && typeof content.focus === 'object' ? content.focus : null,
    // Which PATTERN this day was generated for. Same reasoning as `focus`, same
    // JSONB, no new column — and null for every non-DSA day and every day
    // cached before the pattern roadmap existed, which read back as before.
    pattern: content.pattern && typeof content.pattern === 'object' ? content.pattern : null,
  };
  const empty = meta.estimated_time == null && !meta.difficulty
    && meta.prerequisites.length === 0 && meta.learning_objectives.length === 0
    && !meta.focus && !meta.pattern;
...
/**
 * Is this student on the DSA track?
 *
 * The pattern-based roadmap (lib/dsaPatterns.js) is a C++/DSA syllabus, so it
 * may only reshape days for the `dsa` domain. Routing it anywhere else would
 * resurrect exactly the domain-bleed bug DOMAIN_TRACKS exists to prevent —
 * "LeetCode sliding window" injected into an AI/ML student's Python track.
 * @param {string|null|undefined} domain
 * @returns {boolean}
 */
export function isDsaTrack(domain) {
  return normalizeDomain(domain) === 'dsa';
}
...
//
// These strings are rendered to the student verbatim (see /api/coding/company →
// app/(app)/coding/page.jsx), so they carry only qualitative focus areas. No
// round counts, question counts, section timings or cutoffs: those change per
// hiring cycle and we have no dated source, so stating them would be invention.
const COMPANY_PROFILES = {
  'Amazon': 'Focus: Graphs, Trees, DP, System Design. Leadership Principles matter — prepare concrete behavioral scenarios. Expect LeetCode Medium-Hard difficulty.',
  'Microsoft': 'Focus: Arrays, Strings, Trees, DP. Clean code emphasis. Problem-solving depth, plus a design discussion.',
  'Google': 'Focus: Advanced DSA, Graphs, DP, complexity optimization. Expect LeetCode Hard difficulty and close scrutiny of Big-O.',
  'Flipkart': 'Focus: DSA + Low-level design. Java/system fundamentals. Machine coding round — build working code under time pressure.',
  'TCS': 'Focus: TCS NQT pattern — Quant, Verbal, Logic, and a coding section at easy-to-medium difficulty. OOP concepts. Consistent basics.',
  'Infosys': 'Focus: Infosys SP — Aptitude, coding, OOP, DBMS. Pseudocode questions. Medium difficulty.',
  'Wipro': 'Focus: Wipro NLTH — Aptitude, English, and a coding section spanning easy to hard. Essay writing.',
 ...
 * @param {object} [opts]
 * @param {object} [opts.targeting]  pre-computed roadmap targeting (Phase 3).
 *                                   Passed in by the caller so the readiness
 *                                   read is paid for once per request; omitted
 *                                   means this day is generated untargeted,
 *                                   exactly as before the evidence bus existed.
 */
export async function generateDayContent(domain, dayNumber, level, user = null, opts = {}) {
...
Every task today should move the student closer to clearing ${primaryTarget}'s specific interview process. Include at least one task directly relevant to their interview pattern.`
  ...
  // ── EVIDENCE-DRIVEN FOCUS (Phase 3) ──────────────────────────────────────
  // The named gap from the student's readiness breakdown, if they have enough
  // evidence for one. `buildAdaptationInstructions` above adapts DIFFICULTY
  // from aggregate performance; this adapts SUBJECT from tagged evidence — the
  // difference between "go easier today" and "go easier on Trees today,
  // because 5/16".
  const targeting = opts.targeting || null;
  const gapInstruction = targeting?.targeted ? targeting.instructions : '';

  // ── PATTERN-BASED ORGANIZATION ───────────────────────────────────────────
  // WHICH PATTERN today belongs to. This layers ON TOP of the gap instruction
  // above, it does not replace it: targeting still owns which measured gap the
  // day attacks; this owns the pattern the day sits inside, so a student works
  // one pattern to mastery instead of hopping topics on a day counter.
  //
  // Only ever set for the DSA track (see isDsaTrack) — the caller decides, and
  // an absent plan means the day generates exactly as it did before.
  const patternPlan = opts.patternPlan || null;
  let patternInstruction = '';
  let patternSlice = null;
  if (patternPlan) {
    const { buildPatternInstructions, patternForCache } = await import('./dsaPatternProgress.js');
    patternInstruction = buildPatternInstructions(patternPlan);
    // Rides inside the existing roadmap.meta JSONB, exactly like `focus` — a
    // cached day can then say which pattern it was generated for even after the
    // student has moved on to the next one.
    patternSlice = patternForCache(patternPlan);
  }

  // The RAG lookup follows the gap when there is one, so the reference notes
  // pulled in are about what the student is actually failing.
  // On a pattern-based day the SUBJECT is the pattern, so the reference lookup
  // follows the pattern first, then the gap, then the calendar phase — most
  // specific known subject wins.
  const patternName = patternPlan?.current?.name || null;
  const { searchKnowledgeBase, formatRagContext } = await import('./ragSearch.js');
  const ragQuery = patternName
    ? `${patternName} ${targeting?.targeted ? targeting.focus.label : ''}`.trim()
    : targeting?.targeted ? `${targeting.focus.label} ${phase.focus}` : phase.focus;
  const ragResults = await searchKnowledgeBase(ragQuery, domainSlug, primaryTarget, 3);
  const ragContext = formatRagContext(ragResults, 'PLACEMENT REFERENCE NOTES (prioritize this in your content)');

  const contextualPrefix = (systemContext || ragContext || performanceAdaptation || gapInstruction || patternInstruction)
    ? `${systemContext}\n${urgencyInstruction}\n${companyInstruction}\n${weaknessInstruction}${performanceAdaptation}${patternInstruction}${gapInstruction}${ragContext}\n\n`
    : '';
...
Week: ${week}
${patternName
  ? `Current Pattern: ${patternName} (the day counter is PACE only — the pattern above decides today's subject, and the calendar phase below is context, not an instruction to switch topics)
Calendar phase, for context: ${phase.focus}`
...

{
  ...
  if (!deepseekUrl || !deepseekKey) {
    ...
    for (const model of modelsToTry) {
      try {
        ...
        return withSafeVideo({ ...parsed, focus: focusForCache(targeting), pattern: patternSlice, generated_by: `deepseek (${model})` });
      } catch (e) {
  ...
  try {
    ...
    return withSafeVideo({ ...result, focus: focusForCache(targeting), pattern: patternSlice, generated_by: 'claude' });
  } catch (e) {
    console.error('Claude also failed, using static fallback:', e);
  }

  // Static fallback - always works. It still honours the evidence-driven focus:
  // with both AI providers down, a student with a measured Trees gap gets a
  // Trees day rather than whatever the day counter happened to land on. With a
  // pattern plan it does better still — a real, unsolved problem from the
  // student's current pattern, because that list is hand-authored data and
  // needs no model to serve.
  return withSafeVideo(getStaticFallback(domainSlug, dayNumber, week, phase, isProjectDay, projectThisWeek, targeting, patternPlan));
}

function getStaticFallback(domain, dayNumber, week, phase, isProjectDay, project, targeting = null, patternPlan = null) {
  const topics = {
  ...
  // A measured gap outranks the day counter — but only for a PROVEN weakness.
  // An unmeasured area gets the normal syllabus day, because with no AI to
  // write a calibration task there is nothing here that would measure it.
  const gapTopic = targeting?.targeted && targeting.focus.kind === 'remediate'
    ? targeting.focus.label
    : null;
  const topic = gapTopic || domainTopics[(dayNumber - 1) % domainTopics.length];
  ...
  // The pattern taxonomy is static data, so this path loses nothing when the
  // models are down: the student gets their real current pattern, its authored
  // concept intro, and the next unsolved problem off its list at the easiest
  // difficulty they haven't cleared. No invented topic, no dead LeetCode link.
  const pat = patternPlan?.current || null;
  const nextProblem = pat?.problems?.find(p => !p.solved) || null;

  const fallback = pat ? {
    topic: `${pat.name} — ${nextProblem ? nextProblem.title : 'consolidation'}`,
    description: pat.why,
    objectives: [
      `Work ${pat.name} specifically — not a different pattern`,
      nextProblem ? `Solve ${nextProblem.title} (${nextProblem.difficulty})` : `Re-solve your weakest problem in this pattern from scratch`,
      `Be able to say: ${pat.tell}`,
    ],
    video_url: res.video,
    resource_url: res.resource,
    article_url: res.article,
    coding_problem: nextProblem
      ? `${nextProblem.title} (${nextProblem.difficulty}) — ${pat.name}`
      : `Re-solve a ${pat.name} problem from scratch, no notes`,
    coding_problem_url: nextProblem ? nextProblem.url : res.problem_url,
    estimated_minutes: 90,
    difficulty: nextProblem?.difficulty === 'hard' ? 'hard' : nextProblem?.difficulty === 'medium' ? 'medium' : 'easy',
    prerequisites: pat.prerequisites || [],
    learning_objectives: [pat.tell],
    key_concepts: [pat.name, `${pat.solvedCount}/${pat.needed} problems toward mastery`, `pace: ${patternPlan.pace.problemsPerDay}/day`],
    is_project_day: isProjectDay,
    focus: focusForCache(targeting),
    pattern: { id: pat.id, name: pat.name, state: pat.state, solved: pat.solvedCount, needed: pat.needed, accuracy: pat.evidence?.accuracy ?? null, evidenceTotal: pat.evidence?.total ?? 0 },
    generated_by: 'static_fallback'
  } : {
    topic: `${topic} - Day ${dayNumber}`,
    description: gapTopic
      ? `${topic} is your weakest measured area — ${targeting.focus.correct}/${targeting.focus.total} correct so far. Today rebuilds it from fundamentals.`
      : `Learn ${topic} as part of your ${domain} journey. Today focuses on ${phase.focus}.`,
...
export { DOMAIN_STRUCTURES, COMPANY_PROFILES };
