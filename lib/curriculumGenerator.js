...
// Maps each domain to the language its coding tasks should be written in.
// Without this, the model defaults to C++/DSA idioms for every domain — an
// AI/ML student would get C++ problems instead of Python. Keep keys aligned
// with the validDomains slugs used at signup.
const DOMAIN_LANGUAGE = {
  ...
  devops:        'Python/Bash',
  systemdesign:  'language-agnostic',
};
...
export async function generateDayContent(domain, dayNumber, level, user = null) {
  ...
  const primaryLanguage = DOMAIN_LANGUAGE[domain] || 'the language standard for this domain';
  const languageInstruction = `PRIMARY LANGUAGE for ${domain}: ${primaryLanguage}
Generate all coding tasks and examples in this language.
EXCEPTION: If a company's algorithmic round REQUIRES a different language (e.g. some require C++ for competitive rounds), you MAY override — but you MUST explicitly state: "Note: Using C++ here because [company]'s coding round requires it, even though your domain is ${domain}."
...

{
  ...
  return withSafeVideo(getStaticFallback(domain, dayNumber, week, phase, isProjectDay, projectThisWeek));
}
