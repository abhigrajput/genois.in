...
export async function sendStreakBreakEmail({ to, name, streak, rank, dashboardUrl }) {
  const { data, error } = await getResend().emails.send({
    ...
    html: `<div style="background:#020812;color:#e8f4ff;font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:40px 24px;border-radius:12px;"><div style="font-size:28px;font-weight:800;margin-bottom:8px;"><span style="color:#00f0ff">GEN</span><span style="color:#e8f4ff">OIS</span></div><div style="height:2px;background:linear-gradient(90deg,#ff2d78,transparent);margin-bottom:32px;"></div><div style="font-size:48px;margin-bottom:16px;">🔥</div><h1 style="font-size:24px;font-weight:800;color:#ff2d78;margin-bottom:12px;">Your ${streak}-day streak breaks in 2 hours</h1><p style="color:#8a9ab0;font-size:15px;line-height:1.7;margin-bottom:24px;">Hey ${name}, you have not completed today tasks yet. Your ${streak}-day streak is about to be lost. You are currently ranked <strong style="color:#00f0ff">#${rank}</strong>.</p><a href="${dashboardUrl}" style="display:inline-block;padding:14px 32px;background:linear-gradient(135deg,#00f0ff,#7b5cff);color:#020812;text-decoration:none;border-radius:10px;font-weight:800;font-size:16px;margin-bottom:24px;">Save My Streak Now</a><div style="margin-top:40px;padding-top:20px;border-top:1px solid rgba(255,255,255,0.1);color:#3a4a5a;font-size:12px;">GENOIS · genois.in</div></div>`,
  });
...
export async function sendTrialExpiryEmail({ to, name, daysLeft, score, rank }) {
  const { data, error } = await getResend().emails.send({
    ...
    html: `<div style="background:#020812;color:#e8f4ff;font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:40px 24px;border-radius:12px;"><div style="font-size:28px;font-weight:800;margin-bottom:8px;"><span style="color:#00f0ff">GEN</span><span style="color:#e8f4ff">OIS</span></div><div style="height:2px;background:linear-gradient(90deg,#EF9F27,transparent);margin-bottom:32px;"></div><h1 style="font-size:24px;font-weight:800;color:#EF9F27;margin-bottom:12px;">Your free trial ends in ${daysLeft} day${daysLeft > 1 ? 's' : ''}</h1><p style="color:#8a9ab0;font-size:15px;line-height:1.7;margin-bottom:24px;">Hey ${name}, your trial is ending soon. You have scored <strong style="color:#00f0ff">${score} pts</strong> and reached rank <strong style="color:#7b5cff">#${rank}</strong>.</p><a href="https://genois.in/subscription" style="display:block;text-align:center;padding:14px 32px;background:linear-gradient(135deg,#00f0ff,#7b5cff);color:#020812;text-decoration:none;border-radius:10px;font-weight:800;font-size:16px;margin-bottom:24px;">Upgrade Now from Rs 199 per month</a><div style="margin-top:40px;padding-top:20px;border-top:1px solid rgba(255,255,255,0.1);color:#3a4a5a;font-size:12px;">GENOIS · genois.in</div></div>`,
  });
