...
// FIX P4: the tamper-resistant, XSS-inaccessible transport for the session
// token. HttpOnly so client JS can never read it; SameSite=Lax so it rides
// top-level navigations (middleware gating) but not cross-site fetches/POSTs —
// which, combined with the Origin checks in proxy.js, keeps it CSRF-safe.
export function sessionCookie(token, maxAgeSec = 7 * 24 * 3600) {
  return `genois_token=${token}; Path=/; HttpOnly; ${IS_PROD ? 'Secure; ' : ''}SameSite=Lax; Max-Age=${maxAgeSec}`;
}
...
export async function verifyToken(token) {
  ...
  try {
    ...
    if (await isBlacklisted(token)) {
      console.warn('verifyToken: token rejected by blacklist');
      return null;
    }
    return jwt.verify(token, JWT_SECRET);
  } catch (e) {
    // Log the rejection reason — a silent null here turns any infra failure
    // (Redis outage, secret misconfig, malformed token) into an unexplained 401.
    console.error('verifyToken failed:', e?.name, e?.message);
    return null;
  }
}

// FIX P4: accept the token from the Authorization header OR the httpOnly
// `genois_token` cookie. The cookie is the secure transport set by
// login/signup/refresh; the Bearer header stays supported for existing
// localStorage-based clients during migration.
function extractToken(request) {
  ...
  // Cookie fallback — prefer NextRequest.cookies, fall back to raw header parse.
  const fromNextCookie = request.cookies?.get?.('genois_token')?.value;
  ...
  if (cookieHeader) {
    for (const part of cookieHeader.split(';')) {
      ...
      if (key === 'genois_token') return decodeURIComponent(part.slice(idx + 1).trim());
    }
  }
  return null;
}
