  ...
  try {
    ...
    if (user) {
      ...
      const { error: emailError } = await resend.emails.send({
        ...
        html: `<div style="background:#0a0a0f;color:#e8e8ed;font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:40px 24px;border-radius:12px;"><div style="font-size:28px;font-weight:800;margin-bottom:8px;"><span style="color:#00d9a3">GEN</span><span style="color:#e8e8ed">OIS</span></div><div style="height:2px;background:linear-gradient(90deg,#00d9a3,transparent);margin-bottom:32px;"></div><h1 style="font-size:22px;font-weight:800;color:#e8e8ed;margin-bottom:12px;">Reset your password</h1><p style="color:#8a9ab0;font-size:15px;line-height:1.7;margin-bottom:24px;">Hey ${user.name || 'Student'}, click below to reset your GENOIS password. This link expires in 1 hour.</p><a href="${resetUrl}" style="display:block;text-align:center;padding:14px 32px;background:linear-gradient(135deg,#00d9a3,#ff6b4a);color:#0a0a0f;text-decoration:none;border-radius:10px;font-weight:800;font-size:16px;margin-bottom:24px;">Reset My Password</a><p style="color:#5a7a9a;font-size:13px;">If you did not request this ignore this email.</p><div style="margin-top:40px;padding-top:20px;border-top:1px solid rgba(255,255,255,0.1);color:#3a4a5a;font-size:12px;">GENOIS · genois.in</div></div>`,
      });
      if (emailError) {
