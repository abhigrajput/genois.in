...
export async function POST(request) {
  try {
    ...
    await resend.emails.send({
      ...
      subject: `[${(type || 'FEEDBACK').toUpperCase()}] ${subject || 'New feedback from student'}`,
      html: `
        <div style="background:#020812;color:#e8f4ff;font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:40px 24px;border-radius:12px;">
          ...
          <h2 style="color:#00f0ff;margin-bottom:16px;">New ${type || 'Feedback'} Received</h2>
          <table style="width:100%;border-collapse:collapse;margin-bottom:20px;">
            <tr><td style="padding:8px 0;color:#5a7a9a;width:120px;">From:</td><td style="color:#e8f4ff;">${email || 'Anonymous'}</td></tr>
            <tr><td style="padding:8px 0;color:#5a7a9a;">User ID:</td><td style="color:#e8f4ff;">${payload?.userId || 'Not logged in'}</td></tr>
            <tr><td style="padding:8px 0;color:#5a7a9a;">Type:</td><td style="color:#00f0ff;font-weight:700;">${type || 'general'}</td></tr>
            <tr><td style="padding:8px 0;color:#5a7a9a;">Subject:</td><td style="color:#e8f4ff;">${subject || 'N/A'}</td></tr>
          </table>
          <div style="background:#070f1f;border:1px solid rgba(0,240,255,0.15);border-radius:10px;padding:20px;margin-bottom:20px;">
            ...
            <div style="color:#e8f4ff;font-size:15px;line-height:1.7;">${message}</div>
          </div>
