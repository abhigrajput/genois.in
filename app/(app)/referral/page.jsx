  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto', fontFamily: 'Outfit,sans-serif' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg,rgba(29,158,117,0.1),rgba(0,240,255,0.05))', border: '1px solid rgba(29,158,117,0.3)', borderRadius: 14, padding: 24, marginBottom: 20 }}>
        <div style={{ fontSize: 11, color: '#1D9E75', fontFamily: 'JetBrains Mono,monospace', letterSpacing: 2, marginBottom: 8 }}>YOUR REFERRAL CODE</div>
        <div style={{ fontFamily: 'Syne,sans-serif', fontSize: 32, fontWeight: 800, color: '#1D9E75', letterSpacing: 3, marginBottom: 16 }}>
          {data?.referralCode}
        </div>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          <button onClick={copyLink} style={{ padding: '10px 18px', borderRadius: 10, border: '1px solid rgba(0,240,255,0.3)', background: 'transparent', color: '#00f0ff', cursor: 'pointer', fontSize: 13, fontFamily: 'Syne,sans-serif', fontWeight: 600 }}>
            🔗 Copy Link
          </button>
          <button onClick={shareWhatsApp} style={{ padding: '10px 18px', borderRadius: 10, border: 'none', background: '#25D366', color: '#020812', cursor: 'pointer', fontSize: 13, fontFamily: 'Syne,sans-serif', fontWeight: 700 }}>
            📱 Share WhatsApp
          </button>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12, marginBottom: 24 }}>
        <div style={{ background: '#070f1f', border: '1px solid rgba(0,240,255,0.1)', borderRadius: 12, padding: 20, textAlign: 'center' }}>
          <div style={{ fontFamily: 'Syne,sans-serif', fontSize: 28, fontWeight: 800, color: '#00f0ff' }}>{data?.stats?.total || 0}</div>
          <div style={{ fontSize: 11, color: '#5a7a9a', marginTop: 4, fontFamily: 'JetBrains Mono,monospace' }}>TOTAL REFS</div>
        </div>
        <div style={{ background: '#070f1f', border: '1px solid rgba(29,158,117,0.1)', borderRadius: 12, padding: 20, textAlign: 'center' }}>
          <div style={{ fontFamily: 'Syne,sans-serif', fontSize: 28, fontWeight: 800, color: '#1D9E75' }}>{data?.stats?.converted || 0}</div>
          <div style={{ fontSize: 11, color: '#5a7a9a', marginTop: 4, fontFamily: 'JetBrains Mono,monospace' }}>CONVERTED</div>
        </div>
        <div style={{ background: '#070f1f', border: '1px solid rgba(239,159,39,0.1)', borderRadius: 12, padding: 20, textAlign: 'center' }}>
          <div style={{ fontFamily: 'Syne,sans-serif', fontSize: 28, fontWeight: 800, color: '#EF9F27' }}>{data?.stats?.pending || 0}</div>
          <div style={{ fontSize: 11, color: '#5a7a9a', marginTop: 4, fontFamily: 'JetBrains Mono,monospace' }}>PENDING</div>
        </div>
      </div>
        ...
        {data?.leaderboard?.length > 0 ? data.leaderboard.map((u, i) => (
          <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: i < data.leaderboard.length - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <span style={{ fontSize: 13, color: '#e8f4ff' }}>{u.name || 'Student'}</span>
            </div>
            <div style={{ fontFamily: 'JetBrains Mono,monospace', fontSize: 12, color: '#1D9E75' }}>
