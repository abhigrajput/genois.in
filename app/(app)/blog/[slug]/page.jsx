...
export default function BlogPostDetail({ params }) {
  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '60vh', color: '#5a7a9a', fontFamily: 'Outfit,sans-serif' }}>
  ...
  if (error || !post) {
    return (
      <div style={{ color: '#5a7a9a', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', padding: '60px 24px', fontFamily: 'Outfit,sans-serif' }}>
      ...
      <div style={{ display: 'flex', maxWidth: '1200px', margin: '0 auto', gap: '40px', position: 'relative', zIndex: 1 }}>
        ...
        <article style={{ flex: 1.5, minWidth: 0, maxWidth: '860px' }}>
          <div style={{ display: 'flex', gap: '10px', marginBottom: '16px' }}>
            <span style={{ fontSize: '11px', padding: '4px 10px', borderRadius: '12px', background: `${topicColor}15`, color: topicColor, fontFamily: 'JetBrains Mono,monospace', fontWeight: 'bold', letterSpacing: '0.5px' }}>
              {post.topic?.toUpperCase()}
            </span>
            <span style={{ fontSize: '11px', padding: '4px 10px', borderRadius: '12px', background: `${diffColor}15`, color: diffColor, fontFamily: 'JetBrains Mono,monospace', fontWeight: 'bold' }}>
              {post.difficulty?.toUpperCase()}
            </span>
          </div>

          <h1 style={{ fontFamily: 'Syne,sans-serif', fontSize: '40px', fontWeight: 800, color: '#e8f4ff', margin: '0 0 16px 0', lineHeight: '1.25', letterSpacing: '-1px' }}>
            {post.title}
          </h1>

          {post.excerpt && (
            <p style={{ fontSize: '17px', color: '#8a9ab0', lineHeight: '1.65', margin: '0 0 24px 0', fontFamily: 'Outfit,sans-serif', borderLeft: '4px solid #7b5cff', paddingLeft: '18px' }}>
              {post.excerpt}
            </p>
          )}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', fontSize: '13px', color: '#5a7a9a', marginBottom: '32px', borderTop: '1px solid rgba(255,255,255,0.04)', borderBottom: '1px solid rgba(255,255,255,0.04)', padding: '12px 0' }}>
            <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: 'linear-gradient(135deg,#00f0ff,#7b5cff)', color: '#020812', fontWeight: 'bold', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '13px', fontFamily: 'Syne,sans-serif' }}>
          ...
          {post.cpp_code && (
            <div style={{ marginTop: '48px', paddingTop: '32px', borderTop: '1px solid rgba(255,255,255,0.08)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '12px' }}>
                ...
                <a href={`https://godbolt.org/?code=${encodeURIComponent(post.cpp_code)}`} target="_blank" rel="noreferrer" style={{ textDecoration: 'none', fontSize: '12px', color: '#7b5cff', background: 'rgba(123,92,255,0.08)', border: '1px solid rgba(123,92,255,0.25)', padding: '6px 14px', borderRadius: '8px', fontFamily: 'Syne,sans-serif', fontWeight: 600, transition: 'all 0.15s' }} onMouseOver={(e) => e.target.style.background = 'rgba(123,92,255,0.15)'} onMouseOut={(e) => e.target.style.background = 'rgba(123,92,255,0.08)'}>
          ...
          {post.tags && post.tags.length > 0 && (
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginTop: '40px' }}>
              {post.tags.map((tag, idx) => (
                <span key={idx} style={{ fontSize: '11px', fontFamily: 'JetBrains Mono,monospace', color: '#5a7a9a', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', padding: '4px 10px', borderRadius: '6px' }}>
                  #{tag}
                </span>
              ))}
            </div>
          ...
          <div style={{ marginTop: '48px', background: '#070f1f', border: '1px solid rgba(0,240,255,0.06)', borderRadius: '16px', padding: '24px', display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
            <div style={{ width: '56px', height: '56px', borderRadius: '50%', background: 'linear-gradient(135deg,#00f0ff,#7b5cff)', color: '#020812', fontWeight: 'bold', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '20px', fontFamily: 'Syne,sans-serif', flexShrink: 0 }}>
              {post.authors?.name?.charAt(0) || 'D'}
            </div>
        ...
        <aside style={{ flex: 0.6, minWidth: '240px', display: 'flex', flexDirection: 'column', gap: '24px', height: 'fit-content', position: 'sticky', top: '24px' }}>
          ...
          <div style={{ background: '#070f1f', border: '1px solid rgba(0,240,255,0.06)', borderRadius: '12px', padding: '16px 20px' }}>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <a href={getTwitterShareLink()} target="_blank" rel="noreferrer" style={{ flex: 1, textDecoration: 'none', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '8px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#e8f4ff', fontSize: '13px', fontFamily: 'Syne,sans-serif', fontWeight: 600, transition: 'all 0.15s' }} onMouseOver={(e) => { e.target.style.background = '#1da1f2'; e.target.style.borderColor = '#1da1f2'; }} onMouseOut={(e) => { e.target.style.background = 'rgba(255,255,255,0.02)'; e.target.style.borderColor = 'rgba(255,255,255,0.08)'; }}>
                𝕏 Share
              </a>
