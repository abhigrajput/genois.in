    ...
    <div style={{ fontFamily: 'var(--font-body)', maxWidth: 900, paddingBottom: 60 }}>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 16, flexWrap: 'wrap' }}>
        {['info', 'questions', 'community'].map(t => (
          <button key={t} onClick={() => setTab(t)} style={{ padding: '8px 16px', borderRadius: 8, border: 'none', cursor: 'pointer', background: tab === t ? 'linear-gradient(135deg,#00f0ff,#ff6b4a)' : 'rgba(255,255,255,0.04)', color: tab === t ? '#020812' : '#8a9ab0', fontSize: 13, fontFamily: 'var(--font-heading)', fontWeight: 600, textTransform: 'capitalize' }}>
            {t}
          </button>
        ))}
      </div>
      ...
      {tab === 'info' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ background: '#070f1f', border: '1px solid rgba(0,240,255,0.1)', borderRadius: 12, padding: 18 }}>
            <div style={{ fontFamily: 'var(--font-heading)', fontSize: 14, fontWeight: 700, color: '#00f0ff', marginBottom: 8 }}>Eligibility</div>
            <div style={{ fontSize: 13, color: '#e8e8ed' }}>{c.eligibility}</div>
          </div>
          <div style={{ background: '#070f1f', border: '1px solid rgba(0,240,255,0.1)', borderRadius: 12, padding: 18 }}>
            <div style={{ fontFamily: 'var(--font-heading)', fontSize: 14, fontWeight: 700, color: '#00f0ff', marginBottom: 12 }}>Interview Rounds</div>
            {c.rounds.map((r, i) => (
          ...
          <div style={{ background: '#070f1f', border: '1px solid rgba(0,240,255,0.1)', borderRadius: 12, padding: 18 }}>
            <div style={{ fontFamily: 'var(--font-heading)', fontSize: 14, fontWeight: 700, color: '#00f0ff', marginBottom: 12 }}>Skills Required</div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {c.skills.map((s, i) => (
                <span key={i} style={{ padding: '5px 10px', borderRadius: 16, background: 'rgba(0,240,255,0.06)', color: '#00f0ff', fontSize: 11, fontFamily: 'var(--font-mono)' }}>{s}</span>
              ))}
            </div>
          </div>
          ...
          <div style={{ background: '#070f1f', border: '1px solid rgba(0,240,255,0.1)', borderRadius: 12, padding: 18 }}>
            <div style={{ fontFamily: 'var(--font-heading)', fontSize: 14, fontWeight: 700, color: '#00f0ff', marginBottom: 12 }}>📚 Resources</div>
            {c.resources.map((r, i) => (
      ...
      {tab === 'questions' && (
        <div>
          <button onClick={generateAIQuestions} disabled={generating} style={{ padding: '12px 20px', borderRadius: 10, border: 'none', cursor: generating ? 'not-allowed' : 'pointer', background: 'linear-gradient(135deg,#ff6b4a,#00f0ff)', color: '#020812', fontFamily: 'var(--font-heading)', fontSize: 13, fontWeight: 700, marginBottom: 16 }}>
              ...
              {aiQuestions.map((q, i) => (
                <div key={i} style={{ padding: 14, background: '#070f1f', border: '1px solid rgba(255,107,74,0.15)', borderRadius: 10, marginBottom: 8 }}>
                  <div style={{ display: 'flex', gap: 6, marginBottom: 6 }}>
                    <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 12, background: 'rgba(0,240,255,0.1)', color: '#00f0ff', fontFamily: 'var(--font-mono)' }}>{q.category}</span>
      ...
      {tab === 'community' && (
        <div>
          <div style={{ background: '#070f1f', border: '1px solid rgba(0,240,255,0.1)', borderRadius: 10, padding: 14, marginBottom: 16 }}>
            ...
            <button onClick={postDiscussion} disabled={posting || newDiscussion.length < 5} style={{ marginTop: 8, padding: '8px 16px', borderRadius: 8, border: 'none', cursor: 'pointer', background: 'linear-gradient(135deg,#00f0ff,#ff6b4a)', color: '#020812', fontFamily: 'var(--font-heading)', fontSize: 12, fontWeight: 700 }}>
