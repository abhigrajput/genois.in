...
export default function AuctionPage() {
  ...
  if (loading) return <div style={{ color: '#5a7a9a', padding: 60, textAlign: 'center', fontFamily: 'var(--font-mono)' }}>Loading Auction Block...</div>;
  ...
  return (
    <div style={{ width: '100%', fontFamily: 'var(--font-body)' }}>
      <div style={{ marginBottom: 32 }}>
        <h1 style={{ fontFamily: 'var(--font-heading)', fontSize: 32, fontWeight: 800, color: '#EF9F27', textTransform: 'uppercase', letterSpacing: '-1px', marginBottom: 6 }}>
          ⚡ Skill Auction Block
        </h1>
      ...
      {!myAuction && (
        <div style={{ background: '#070f1f', border: '1px solid rgba(239,159,39,0.2)', borderRadius: 16, padding: '32px', marginBottom: 32, display: 'flex', flexWrap: 'wrap', gap: 24, alignItems: 'center' }}>
          <div style={{ flex: 1, minWidth: 260 }}>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: '#EF9F27', letterSpacing: 2, marginBottom: 8 }}>YOUR STATUS</div>
            <div style={{ fontFamily: 'var(--font-heading)', fontSize: 24, fontWeight: 800, color: '#e8e8ed', marginBottom: 8 }}>
          ...
          {isEligible && (
            <div style={{ background: 'rgba(239,159,39,0.05)', padding: '20px', borderRadius: 12, border: '1px solid rgba(239,159,39,0.1)', width: 280 }}>
              <div style={{ fontSize: 11, color: '#5a7a9a', fontFamily: 'var(--font-mono)', marginBottom: 6 }}>MINIMUM BASE CTC (₹)</div>
              <input type="number" value={minBid} onChange={e => setMinBid(e.target.value)} style={{ width: '100%', padding: '10px', borderRadius: 8, border: '1px solid rgba(239,159,39,0.3)', background: 'transparent', color: '#EF9F27', fontSize: 16, fontFamily: 'var(--font-mono)', outline: 'none', marginBottom: 12, boxSizing: 'border-box' }} />
              <button disabled={listing} onClick={listMyself} style={{ width: '100%', padding: '12px', background: '#EF9F27', color: '#020812', border: 'none', borderRadius: 8, fontFamily: 'var(--font-heading)', fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>
      ...
      {myAuction && (
        <div style={{ background: 'linear-gradient(135deg,rgba(239,159,39,0.1),rgba(0,0,0,0))', border: '1px solid rgba(239,159,39,0.3)', borderRadius: 16, padding: '24px', marginBottom: 32 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: '#EF9F27', letterSpacing: 2, marginBottom: 4 }}>YOU ARE ON THE BLOCK</div>
              <div style={{ fontFamily: 'var(--font-heading)', fontSize: 32, fontWeight: 800, color: '#e8e8ed' }}>₹{myAuction.current_bid.toLocaleString('en-IN')}</div>
              <div style={{ fontSize: 12, color: '#5a7a9a', marginTop: 4 }}>Highest Bid ({myAuction.bid_count} total bids)</div>
            </div>
            <div style={{ textAlign: 'right' }}>
              <span style={{ padding: '6px 14px', background: 'rgba(29,158,117,0.15)', color: '#1D9E75', borderRadius: 20, fontSize: 12, fontFamily: 'var(--font-mono)', fontWeight: 700 }}>
                ACTIVE
              </span>
            </div>
          </div>
      ...
      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: '#5a7a9a', letterSpacing: 2, marginBottom: 16 }}>LIVE AUCTIONS ({auctions.length})</div>
          ...
          <div key={a.id} style={{ background: '#070f1f', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 14, padding: '20px', position: 'relative' }}>
            <div style={{ display: 'flex', alignItems: 'start', justifyContent: 'space-between', marginBottom: 12 }}>
              ...
              <div style={{ background: 'rgba(239,159,39,0.1)', color: '#EF9F27', padding: '10px 14px', borderRadius: 10, textAlign: 'center' }}>
                ...
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: 9, opacity: 0.8 }}>Current Bid</div>
              </div>
            </div>
            
            <button onClick={() => { setBidModal(a); setRecruiterForm({ ...recruiterForm, bid: a.current_bid + 50000 }); }} style={{ width: '100%', padding: '10px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.1)', color: '#e8e8ed', borderRadius: 8, cursor: 'pointer', fontFamily: 'var(--font-mono)', fontSize: 11, transition: 'background 0.2s' }}>
      ...
      {bidModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: '#0a1628', border: '1px solid rgba(239,159,39,0.3)', borderRadius: 16, padding: '28px', maxWidth: 400, width: '100%' }}>
            <div style={{ fontFamily: 'var(--font-heading)', fontSize: 20, fontWeight: 800, color: '#e8e8ed', marginBottom: 4 }}>Bid on {bidModal.name}</div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 24 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(239,159,39,0.3)', borderRadius: 8, padding: '0 12px' }}>
                <span style={{ color: '#EF9F27', fontFamily: 'var(--font-mono)' }}>₹</span>
                <input type="number" value={recruiterForm.bid} onChange={e => setRecruiterForm({...recruiterForm, bid: parseInt(e.target.value) || 0})} placeholder="Bid Amount" style={{ flex: 1, padding: '12px 0', background: 'transparent', border: 'none', color: '#EF9F27', fontFamily: 'var(--font-mono)', fontSize: 16, outline: 'none' }} />
              </div>
