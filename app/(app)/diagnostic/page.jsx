...
export default function DiagnosticPage() {
  ...
  if (phase === 'result' && result) {
    ...
    return (
      <div style={{ maxWidth: 680, margin: '0 auto', fontFamily: 'Outfit,sans-serif', textAlign: 'center' }}>
        <div style={{ marginBottom: 32 }}>
          <div style={{ fontFamily: 'JetBrains Mono,monospace', fontSize: 11, color: '#5a7a9a', letterSpacing: 2, marginBottom: 12 }}>
            DIAGNOSTIC COMPLETE
          </div>
          <div style={{ fontFamily: 'Syne,sans-serif', fontSize: 64, fontWeight: 800, color, marginBottom: 8 }}>
            {result.score}%
          </div>
          <div style={{ fontFamily: 'Syne,sans-serif', fontSize: 20, fontWeight: 700, color, marginBottom: 8 }}>
        ...
        <div style={{ background: '#070f1f', border: `1px solid ${color}20`, borderRadius: 14, padding: 24, marginBottom: 24, textAlign: 'left' }}>
          <div style={{ fontFamily: 'JetBrains Mono,monospace', fontSize: 10, color: '#5a7a9a', letterSpacing: 2, marginBottom: 16 }}>WHAT THIS MEANS</div>
        ...
        <div style={{ background: '#070f1f', border: '1px solid rgba(0,240,255,0.08)', borderRadius: 14, padding: 20, marginBottom: 24 }}>
          <div style={{ fontFamily: 'JetBrains Mono,monospace', fontSize: 10, color: '#00f0ff', letterSpacing: 2, marginBottom: 12 }}>YOUR BASELINE SCORE</div>
        ...
        <button onClick={() => router.push('/roadmap')} style={{ width: '100%', padding: '16px', borderRadius: 12, border: 'none', cursor: 'pointer', background: 'linear-gradient(135deg,#00f0ff,#7b5cff)', color: '#020812', fontFamily: 'Syne,sans-serif', fontSize: 16, fontWeight: 800 }}>
      ...
      <div style={{ maxWidth: 680, margin: '0 auto', fontFamily: 'Outfit,sans-serif' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 20, background: `${diffColors[q.difficulty]}15`, color: diffColors[q.difficulty], fontFamily: 'JetBrains Mono,monospace' }}>
              {q.difficulty?.toUpperCase()}
            </span>
            <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 20, background: 'rgba(255,255,255,0.04)', color: '#5a7a9a', fontFamily: 'JetBrains Mono,monospace' }}>
              {q.points} pts
            </span>
          </div>
        </div>
        ...
        <div style={{ height: 4, background: 'rgba(255,255,255,0.06)', borderRadius: 2, marginBottom: 24, overflow: 'hidden' }}>
          <div style={{ height: '100%', width: `${((currentQ + 1) / questions.length) * 100}%`, background: 'linear-gradient(90deg,#00f0ff,#7b5cff)', borderRadius: 2, transition: 'width 0.3s' }} />
        </div>

        <div style={{ background: '#070f1f', border: '1px solid rgba(0,240,255,0.1)', borderRadius: 14, padding: 24, marginBottom: 20 }}>
          <div style={{ fontFamily: 'JetBrains Mono,monospace', fontSize: 10, color: '#5a7a9a', letterSpacing: 2, marginBottom: 10 }}>
            {q.topic?.toUpperCase()}
          </div>
          <div style={{ fontSize: 17, color: '#e8f4ff', lineHeight: 1.7, fontWeight: 500 }}>{q.question}</div>
        </div>
          ...
          {(q.options || []).map((opt, oi) => {
            ...
            return (
              <button key={oi} onClick={() => setAnswers(p => ({ ...p, [currentQ]: opt }))} style={{
                ...
                <span style={{ color: isSelected ? '#00f0ff' : '#5a7a9a', marginRight: 10, fontFamily: 'JetBrains Mono,monospace', fontWeight: 700 }}>{letters[oi]}.</span>
                {opt}
              </button>
            );
          })}
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          {currentQ > 0 && (
            <button onClick={() => setCurrentQ(q => q - 1)} style={{ flex: 1, padding: '13px', borderRadius: 12, border: '1px solid rgba(255,255,255,0.1)', background: 'transparent', color: '#5a7a9a', cursor: 'pointer', fontFamily: 'Syne,sans-serif', fontSize: 14 }}>← Back</button>
          )}
          {currentQ < questions.length - 1 ? (
            <button onClick={() => { if (!answers[currentQ]) { toast.error('Select an answer first'); return; } setCurrentQ(q => q + 1); }} style={{ flex: 2, padding: '13px', borderRadius: 12, border: 'none', cursor: 'pointer', background: answers[currentQ] ? 'linear-gradient(135deg,#00f0ff,#7b5cff)' : 'rgba(255,255,255,0.05)', color: answers[currentQ] ? '#020812' : '#3a4a5a', fontFamily: 'Syne,sans-serif', fontSize: 14, fontWeight: 700 }}>
              Next →
            </button>
          ) : (
            <button onClick={submitTest} disabled={submitting} style={{ flex: 2, padding: '13px', borderRadius: 12, border: 'none', cursor: 'pointer', background: submitting ? 'rgba(0,240,255,0.2)' : 'linear-gradient(135deg,#00f0ff,#7b5cff)', color: '#020812', fontFamily: 'Syne,sans-serif', fontSize: 14, fontWeight: 700 }}>
  ...
  return (
    <div style={{ maxWidth: 620, margin: '0 auto', fontFamily: 'Outfit,sans-serif', textAlign: 'center' }}>
      <div style={{ marginBottom: 32 }}>
        <div style={{ fontSize: 64, marginBottom: 16 }}>🎯</div>
        <h1 style={{ fontFamily: 'Syne,sans-serif', fontSize: 26, fontWeight: 800, color: '#e8f4ff', marginBottom: 12 }}>
      ...
      <div style={{ background: '#070f1f', border: '1px solid rgba(0,240,255,0.1)', borderRadius: 14, padding: 24, marginBottom: 28, textAlign: 'left' }}>
        <div style={{ fontFamily: 'JetBrains Mono,monospace', fontSize: 10, color: '#00f0ff', letterSpacing: 2, marginBottom: 16 }}>WHAT TO EXPECT</div>
        {[
      ...
      <button onClick={startTest} disabled={generating} style={{ width: '100%', padding: '16px', borderRadius: 12, border: 'none', cursor: 'pointer', background: generating ? 'rgba(0,240,255,0.2)' : 'linear-gradient(135deg,#00f0ff,#7b5cff)', color: '#020812', fontFamily: 'Syne,sans-serif', fontSize: 16, fontWeight: 800, marginBottom: 12, boxShadow: '0 0 30px rgba(0,240,255,0.2)' }}>
