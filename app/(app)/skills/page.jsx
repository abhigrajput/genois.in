...
export default function SkillsPage() {
  ...
  if (loading) return <div style={{ color: '#5a7a9a', padding: 60, textAlign: 'center', fontFamily: 'JetBrains Mono,monospace' }}>Loading...</div>;

  if (phase === 'loading') return (
    <div style={{ color: '#5a7a9a', padding: 60, textAlign: 'center', fontFamily: 'JetBrains Mono,monospace' }}>
      <div style={{ fontSize: 48, marginBottom: 16 }}>⚒️</div>
      <div style={{ fontSize: 16, marginBottom: 8, color: '#e8f4ff', fontFamily: 'Syne,sans-serif', fontWeight: 700 }}>
    ...
    return (
      <div style={{ maxWidth: 760, margin: '0 auto', fontFamily: 'Outfit,sans-serif' }}>
        <div style={{ background: '#070f1f', border: `1px solid ${result.verified ? (badge?.color || '#1D9E75') + '40' : 'rgba(255,45,120,0.2)'}`, borderRadius: 16, padding: 28, marginBottom: 20, textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ fontFamily: 'Syne,sans-serif', fontSize: 52, fontWeight: 800, color: result.verified ? (badge?.color || '#1D9E75') : '#ff2d78', lineHeight: 1, marginBottom: 8 }}>
            {result.score}%
          </div>
          <div style={{ fontFamily: 'Syne,sans-serif', fontSize: 20, fontWeight: 700, color: '#e8f4ff', marginBottom: 4 }}>
          ...
          {result.verified && (
            <div style={{ display: 'inline-flex', gap: 12, marginTop: 8 }}>
              <div style={{ padding: '6px 14px', borderRadius: 20, background: 'rgba(29,158,117,0.15)', border: '1px solid rgba(29,158,117,0.3)', color: '#1D9E75', fontSize: 12, fontFamily: 'JetBrains Mono,monospace' }}>
                +{result.pointsEarned} pts
              </div>
              <div style={{ padding: '6px 14px', borderRadius: 20, background: 'rgba(255,255,255,0.04)', color: '#5a7a9a', fontSize: 12, fontFamily: 'JetBrains Mono,monospace' }}>
                Expires: {new Date(result.expiresAt).toLocaleDateString('en-IN')}
              </div>
            </div>
          )}
          {result.allLevelsComplete && (
            <div style={{ marginTop: 20, padding: '14px 20px', background: 'linear-gradient(135deg,rgba(255,215,0,0.1),rgba(192,192,192,0.05))', borderRadius: 12, color: '#ffd700', fontFamily: 'Syne,sans-serif', fontSize: 16, fontWeight: 700 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }}>
            {result.strongAreas?.length > 0 && (
              <div style={{ background: '#070f1f', border: '1px solid rgba(29,158,117,0.15)', borderRadius: 12, padding: 16 }}>
                <div style={{ fontFamily: 'JetBrains Mono,monospace', fontSize: 10, color: '#1D9E75', letterSpacing: 2, marginBottom: 10 }}>✓ STRONG</div>
            ...
            {result.weakAreas?.length > 0 && (
              <div style={{ background: '#070f1f', border: '1px solid rgba(255,45,120,0.15)', borderRadius: 12, padding: 16 }}>
                <div style={{ fontFamily: 'JetBrains Mono,monospace', fontSize: 10, color: '#ff2d78', letterSpacing: 2, marginBottom: 10 }}>✗ STUDY THESE</div>
        ...
        <div style={{ background: '#070f1f', border: '1px solid rgba(0,240,255,0.06)', borderRadius: 12, padding: 16, marginBottom: 20 }}>
          <button onClick={() => setShowExplanation(!showExplanation)} style={{ width: '100%', background: 'transparent', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 0 }}>
            <div style={{ fontFamily: 'JetBrains Mono,monospace', fontSize: 10, color: '#5a7a9a', letterSpacing: 2 }}>
            ...
            <div style={{ marginTop: 14, display: 'flex', flexDirection: 'column', gap: 12 }}>
              {result.detailedResults?.map((r, i) => (
                <div key={i} style={{ padding: '12px 14px', borderRadius: 8, background: r.isCorrect ? 'rgba(29,158,117,0.04)' : 'rgba(255,45,120,0.04)', border: `1px solid ${r.isCorrect ? 'rgba(29,158,117,0.15)' : 'rgba(255,45,120,0.15)'}` }}>
                  ...
                  {r.code && (
                    <pre style={{ background: '#050d1a', padding: '8px 12px', borderRadius: 6, fontSize: 11, color: '#00f0ff', fontFamily: 'JetBrains Mono,monospace', margin: '6px 0 6px 22px', overflowX: 'auto' }}>{r.code}</pre>
                  )}
                  {!r.isCorrect && (
                    <>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={() => { setPhase('list'); setSelectedSkill(null); setDetailSkill(null); }} style={{ flex: 1, padding: '12px', borderRadius: 10, border: '1px solid rgba(255,255,255,0.1)', background: 'transparent', color: '#5a7a9a', cursor: 'pointer', fontFamily: 'Syne,sans-serif', fontSize: 14 }}>Back</button>
          {result.nextLevelUnlocked && (
            <button onClick={() => { const next = result.level + 1; setTimeout(() => startLevel(selectedSkill, next), 100); }} style={{ flex: 2, padding: '12px', borderRadius: 10, border: 'none', cursor: 'pointer', background: 'linear-gradient(135deg,#00f0ff,#7b5cff)', color: '#020812', fontFamily: 'Syne,sans-serif', fontSize: 14, fontWeight: 700 }}>
              Start Level {result.level + 1} →
            </button>
          )}
          {!result.verified && result.attemptsLeft > 0 && (
            <button onClick={() => startLevel(selectedSkill, result.level)} style={{ flex: 2, padding: '12px', borderRadius: 10, border: 'none', cursor: 'pointer', background: 'linear-gradient(135deg,#00f0ff,#7b5cff)', color: '#020812', fontFamily: 'Syne,sans-serif', fontSize: 14, fontWeight: 700 }}>
    ...
    return (
      <div style={{ maxWidth: 760, margin: '0 auto', fontFamily: 'Outfit,sans-serif' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, flexWrap: 'wrap', gap: 8 }}>
          ...
          <span style={{ fontSize: 10, padding: '3px 10px', borderRadius: 20, background: 'rgba(0,240,255,0.08)', color: '#00f0ff', fontFamily: 'JetBrains Mono,monospace' }}>
        ...
        <div style={{ background: '#070f1f', border: '1px solid rgba(0,240,255,0.1)', borderRadius: 14, padding: 24, marginBottom: 16 }}>
          <div style={{ fontSize: 16, color: '#e8f4ff', lineHeight: 1.75, fontWeight: 500, marginBottom: q.code ? 16 : 0 }}>{q.question}</div>
          {q.code && (
            <pre style={{ background: '#050d1a', border: '1px solid rgba(0,240,255,0.1)', borderRadius: 8, padding: '14px 16px', fontSize: 13, color: '#00f0ff', fontFamily: 'JetBrains Mono,monospace', overflowX: 'auto', margin: 0, lineHeight: 1.6 }}>{q.code}</pre>
          )}
        </div>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          {currentQ < questions.length - 1 ? (
            ...
            <button onClick={submitLevel} disabled={submitting} style={{ flex: 2, padding: '13px', borderRadius: 12, border: 'none', cursor: submitting ? 'not-allowed' : 'pointer', background: submitting ? 'rgba(0,240,255,0.2)' : 'linear-gradient(135deg,#1D9E75,#00f0ff)', color: '#020812', fontFamily: 'Syne,sans-serif', fontSize: 14, fontWeight: 700 }}>
    ...
    return (
      <div style={{ maxWidth: 720, margin: '0 auto', fontFamily: 'Outfit,sans-serif' }}>
        ...
        <div style={{ background: '#070f1f', border: `1px solid ${color}25`, borderRadius: 16, padding: 24, marginBottom: 20 }}>
          <div style={{ fontFamily: 'JetBrains Mono,monospace', fontSize: 10, color, letterSpacing: 2, marginBottom: 8 }}>MASTERY TRIALS</div>
          <div style={{ fontFamily: 'Syne,sans-serif', fontSize: 28, fontWeight: 800, color: '#e8f4ff', marginBottom: 4 }}>{detailSkill.name}</div>
            ...
            return (
              <div key={lvl.level} style={{ background: '#070f1f', border: `2px solid ${isVerified ? (badge?.color || '#1D9E75') + '50' : isExpired ? 'rgba(239,159,39,0.3)' : locked ? 'rgba(255,255,255,0.04)' : 'rgba(0,240,255,0.15)'}`, borderRadius: 14, padding: 20, opacity: locked ? 0.5 : 1, position: 'relative', overflow: 'hidden' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12, flexWrap: 'wrap', gap: 10 }}>
                  ...
                  {isVerified && (
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ fontSize: 14, fontWeight: 700, color: badge?.color, fontFamily: 'Syne,sans-serif' }}>{lvl.score}%</div>
                ...
                {locked ? (
                  ...
                  <button onClick={() => startLevel(detailSkill, lvl.level)} style={{ width: '100%', padding: '11px', borderRadius: 10, border: 'none', cursor: 'pointer', background: isVerified && !isExpired ? 'rgba(255,255,255,0.04)' : 'linear-gradient(135deg,#00f0ff,#7b5cff)', color: isVerified && !isExpired ? '#5a7a9a' : '#020812', fontFamily: 'Syne,sans-serif', fontSize: 13, fontWeight: 700 }}>
        ...
        {detailSkill.allPassed && (
          <div style={{ marginTop: 20, padding: 20, background: 'linear-gradient(135deg,rgba(255,215,0,0.08),rgba(192,192,192,0.03))', border: '1px solid rgba(255,215,0,0.2)', borderRadius: 14, textAlign: 'center' }}>
            ...
            <div style={{ fontFamily: 'Syne,sans-serif', fontSize: 18, fontWeight: 800, color: '#ffd700', marginBottom: 4 }}>Master of {detailSkill.name}</div>
  ...
  return (
    <div style={{ fontFamily: 'Outfit,sans-serif', width: '100%' }}>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontFamily: 'Syne,sans-serif', fontSize: 26, fontWeight: 800, color: '#e8f4ff', marginBottom: 4 }}>⚒️ Mastery Trials</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(160px,1fr))', gap: 12, marginBottom: 24 }}>
        {[
          ...
          { label: 'Badge Expiry', value: '3 months', color: '#7b5cff', icon: '⏰' },
        ].map(s => (
          <div key={s.label} style={{ background: '#070f1f', border: `1px solid ${s.color}15`, borderRadius: 12, padding: '14px', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 11, color: '#5a7a9a', marginTop: 2, fontFamily: 'JetBrains Mono,monospace' }}>{s.label}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 20 }}>
        {categories.map(cat => (
          <button key={cat} onClick={() => setFilter(cat)} style={{ padding: '6px 14px', borderRadius: 20, border: 'none', cursor: 'pointer', fontFamily: 'Syne,sans-serif', fontSize: 12, fontWeight: 600, background: filter === cat ? '#00f0ff' : 'rgba(255,255,255,0.05)', color: filter === cat ? '#020812' : '#5a7a9a' }}>
            {cat === 'all' ? 'All' : cat}
          </button>
        ))}
      </div>
          ...
          return (
            <div key={i} onClick={() => setDetailSkill(skill)} style={{ background: '#070f1f', border: `2px solid ${skill.allPassed ? '#ffd700' + '40' : skill.highestPassed > 0 ? color + '30' : 'rgba(255,255,255,0.06)'}`, borderRadius: 12, padding: '18px', cursor: 'pointer', position: 'relative', overflow: 'hidden' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 20, background: `${color}15`, color, fontFamily: 'JetBrains Mono,monospace' }}>{skill.category}</span>
                {skill.allPassed && <span style={{ fontSize: 16 }}>🏆</span>}
              </div>
              <div style={{ fontFamily: 'Syne,sans-serif', fontSize: 15, fontWeight: 700, color: '#e8f4ff', marginBottom: 10 }}>{skill.name}</div>
                ...
                {skill.levels.map(lvl => {
                  ...
                  return (
                    <div key={lvl.level} style={{ flex: 1, padding: '6px 4px', borderRadius: 6, background: active ? (badge?.color || '#1D9E75') + '15' : lvl.isExpired ? 'rgba(239,159,39,0.08)' : 'rgba(255,255,255,0.03)', border: `1px solid ${active ? (badge?.color || '#1D9E75') + '30' : 'rgba(255,255,255,0.04)'}`, textAlign: 'center' }}>
                      ...
                      <div style={{ fontSize: 9, color: active ? badge?.color : '#5a7a9a', fontFamily: 'JetBrains Mono,monospace', marginTop: 2 }}>L{lvl.level}</div>
                    </div>
                  );
                })}
              </div>
