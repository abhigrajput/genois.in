...
export default function InterviewSimulatorPage() {
  ...
  if (phase === 'loading') return (
    <div style={{ color: '#5a7a9a', padding: 60, textAlign: 'center', fontFamily: 'var(--font-mono)' }}>
      ...
      <div style={{ fontSize: 16, color: '#e8e8ed', fontFamily: 'var(--font-heading)', fontWeight: 700 }}>Preparing your interview...</div>
    ...
    return (
      <div style={{ maxWidth: 800, margin: '0 auto', fontFamily: 'var(--font-body)' }}>
        ...
        <div style={{ background: `linear-gradient(135deg,${isPass ? 'rgba(29,158,117,0.1)' : 'rgba(255,45,120,0.08)'},transparent)`, border: `2px solid ${isPass ? 'rgba(29,158,117,0.3)' : 'rgba(255,45,120,0.2)'}`, borderRadius: 16, padding: 32, marginBottom: 20, textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ fontFamily: 'var(--font-heading)', fontSize: 36, fontWeight: 800, color: isPass ? '#1D9E75' : '#ff2d78', marginBottom: 8 }}>
          ...
          <div style={{ marginTop: 16, display: 'inline-block', padding: '10px 24px', borderRadius: 20, background: 'rgba(255,255,255,0.06)' }}>
            <span style={{ fontFamily: 'var(--font-heading)', fontSize: 28, fontWeight: 800, color: isPass ? '#1D9E75' : '#ff2d78' }}>{finalResult.overallScore}%</span>
            <span style={{ fontSize: 12, color: '#5a7a9a', marginLeft: 6, fontFamily: 'var(--font-mono)' }}>overall</span>
          </div>
            ...
            {finalResult.roundBreakdown?.map((r, i) => {
              ...
              return (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '10px 14px', background: 'rgba(255,255,255,0.02)', borderRadius: 8 }}>
                  <div style={{ fontSize: 13, color: '#e8e8ed', fontWeight: 600, flex: 1 }}>{r.round}</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(260px,1fr))', gap: 12, marginBottom: 16 }}>
          {finalResult.topStrengths?.length > 0 && (
            <div style={{ background: '#070f1f', border: '1px solid rgba(29,158,117,0.15)', borderRadius: 12, padding: 18 }}>
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: '#1D9E75', letterSpacing: 2, marginBottom: 10 }}>✓ STRENGTHS</div>
          ...
          {finalResult.topWeaknesses?.length > 0 && (
            <div style={{ background: '#070f1f', border: '1px solid rgba(255,45,120,0.15)', borderRadius: 12, padding: 18 }}>
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: '#ff2d78', letterSpacing: 2, marginBottom: 10 }}>✗ IMPROVE</div>
        ...
        {finalResult.actionItems?.length > 0 && (
          <div style={{ background: '#070f1f', border: '1px solid rgba(239,159,39,0.15)', borderRadius: 12, padding: 18, marginBottom: 20 }}>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: '#EF9F27', letterSpacing: 2, marginBottom: 12 }}>ACTION ITEMS — WORK ON THESE</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(240px,1fr))', gap: 12, marginBottom: 20 }}>
            {roundResult.strengths?.length > 0 && (
              <div style={{ background: '#070f1f', border: '1px solid rgba(29,158,117,0.15)', borderRadius: 10, padding: 16 }}>
                <div style={{ fontSize: 11, color: '#1D9E75', marginBottom: 8, fontFamily: 'var(--font-mono)' }}>WHAT WORKED</div>
            ...
            {roundResult.weaknesses?.length > 0 && (
              <div style={{ background: '#070f1f', border: '1px solid rgba(255,45,120,0.15)', borderRadius: 10, padding: 16 }}>
                <div style={{ fontSize: 11, color: '#ff2d78', marginBottom: 8, fontFamily: 'var(--font-mono)' }}>WHAT DID NOT</div>
    ...
    return (
      <div style={{ maxWidth: 820, margin: '0 auto', fontFamily: 'var(--font-body)' }}>
        ...
        <div style={{ background: `linear-gradient(135deg,${companyConfig?.color}10,transparent)`, border: `1px solid ${companyConfig?.color}25`, borderRadius: 14, padding: '14px 20px', marginBottom: 16, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
          <div>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: '#5a7a9a', letterSpacing: 2, marginBottom: 4 }}>
        ...
        <div style={{ height: 5, background: 'rgba(255,255,255,0.06)', borderRadius: 3, marginBottom: 20, overflow: 'hidden' }}>
          <div style={{ height: '100%', width: `${((currentQ + 1) / questions.length) * 100}%`, background: `linear-gradient(90deg,${companyConfig?.color},#ff6b4a)`, borderRadius: 3, transition: 'width 0.3s' }} />
        </div>

        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: '#5a7a9a', letterSpacing: 2, marginBottom: 12 }}>
  ...
  return (
    ...
    <div style={{ fontFamily: 'var(--font-body)', width: '100%' }}>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontFamily: 'var(--font-heading)', fontSize: 26, fontWeight: 800, color: '#e8e8ed', marginBottom: 4 }}>🎯 Interview Simulator</h1>
          ...
          return (
            <div key={key} onClick={() => { setSelectedType(key); setSelectedCompany(c.examples[0]); }} style={{ background: '#070f1f', border: `2px solid ${selected ? c.color + '50' : c.color + '15'}`, borderRadius: 14, padding: 20, cursor: 'pointer', position: 'relative', overflow: 'hidden' }}>
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                {c.rounds.map((r, i) => <span key={i} style={{ fontSize: 9, padding: '2px 8px', borderRadius: 10, background: `${c.color}10`, color: c.color, fontFamily: 'var(--font-mono)' }}>{i + 1}. {r.replace('_', ' ')}</span>)}
              </div>
            </div>
          );
        })}
      ...
      {selectedType && (
        <>
          ...
          <button onClick={startInterview} style={{ width: '100%', padding: '16px', borderRadius: 12, border: 'none', cursor: 'pointer', background: `linear-gradient(135deg,${types[selectedType].color},${types[selectedType].color}99)`, color: '#020812', fontFamily: 'var(--font-heading)', fontSize: 15, fontWeight: 700 }}>
      ...
      {data?.sessions?.length > 0 && (
        <div style={{ marginTop: 32 }}>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: '#5a7a9a', letterSpacing: 2, marginBottom: 14 }}>YOUR PAST INTERVIEWS</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {data.sessions.slice(0, 5).map((s, i) => (
              <div key={i} style={{ background: '#070f1f', border: '1px solid rgba(255,255,255,0.04)', borderRadius: 10, padding: '12px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
                <div>
                  <div style={{ fontSize: 13, color: '#e8e8ed', fontWeight: 600 }}>{s.company_name}</div>
                  <div style={{ fontSize: 11, color: '#5a7a9a', fontFamily: 'var(--font-mono)' }}>{new Date(s.started_at).toLocaleDateString('en-IN')}</div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  {s.completed ? (
                    <>
                      <span style={{ fontFamily: 'var(--font-heading)', fontSize: 16, fontWeight: 800, color: s.verdict === 'SELECTED' ? '#1D9E75' : '#ff2d78' }}>{s.overall_score}%</span>
                      <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 10, background: s.verdict === 'SELECTED' ? 'rgba(29,158,117,0.1)' : 'rgba(255,45,120,0.1)', color: s.verdict === 'SELECTED' ? '#1D9E75' : '#ff2d78', fontFamily: 'var(--font-mono)' }}>{s.verdict}</span>
                    </>
                  ) : (
                    <span style={{ fontSize: 11, color: '#EF9F27', fontFamily: 'var(--font-mono)' }}>INCOMPLETE</span>
                  )}
                </div>
              </div>
            ))}
