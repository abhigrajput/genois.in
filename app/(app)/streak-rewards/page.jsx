...
export default function StreakRewardsPage() {
  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto', fontFamily: 'Outfit,sans-serif', paddingBottom: 40 }}>
      ...
      <div style={{ background: 'linear-gradient(135deg,rgba(239,159,39,0.1),rgba(255,45,120,0.05))', border: '1px solid rgba(239,159,39,0.3)', borderRadius: 14, padding: 24, marginBottom: 20, textAlign: 'center' }}>
        <div style={{ fontFamily: 'JetBrains Mono,monospace', fontSize: 11, color: '#EF9F27', letterSpacing: 2, marginBottom: 8 }}>CURRENT STREAK</div>
        <div style={{ fontFamily: 'Syne,sans-serif', fontSize: 56, fontWeight: 900, background: 'linear-gradient(135deg,#EF9F27,#ff2d78)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', lineHeight: 1 }}>
          {data.currentStreak}
        </div>
      ...
      {data.unclaimed.length > 0 && (
        <div style={{ background: 'linear-gradient(135deg,rgba(0,240,255,0.08),rgba(123,92,255,0.04))', border: '1px solid rgba(0,240,255,0.3)', borderRadius: 14, padding: 24, marginBottom: 20 }}>
          <div style={{ fontFamily: 'Syne,sans-serif', fontSize: 16, fontWeight: 700, color: '#00f0ff', marginBottom: 12 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16 }}>
            {data.unclaimed.map((r, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: 10, background: 'rgba(255,255,255,0.02)', borderRadius: 8 }}>
                ...
                <span style={{ flex: 1, color: '#e8f4ff', fontSize: 13 }}>Day {r.day} — {r.label}</span>
              </div>
            ))}
          </div>
          <button onClick={claimAll} disabled={claiming} style={{ width: '100%', padding: 12, borderRadius: 10, border: 'none', cursor: claiming ? 'wait' : 'pointer', background: 'linear-gradient(135deg,#00f0ff,#7b5cff)', color: '#020812', fontFamily: 'Syne,sans-serif', fontSize: 14, fontWeight: 700 }}>
      ...
      {data.nextReward && (
        <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 14, padding: 20, marginBottom: 20 }}>
          <div style={{ fontFamily: 'JetBrains Mono,monospace', fontSize: 10, color: '#5a7a9a', letterSpacing: 2, marginBottom: 8 }}>NEXT MILESTONE</div>
          <div style={{ fontSize: 14, color: '#e8f4ff', marginBottom: 4 }}>
      ...
      <div style={{ background: '#070f1f', border: '1px solid rgba(255,255,255,0.04)', borderRadius: 14, padding: 20 }}>
        <div style={{ fontFamily: 'Syne,sans-serif', fontSize: 14, fontWeight: 700, color: '#e8f4ff', marginBottom: 14 }}>
          All Milestones
        </div>
          ...
          {data.allRewards.map((r, i) => {
            ...
            return (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: 10, borderRadius: 8, background: achieved ? 'rgba(29,158,117,0.06)' : 'rgba(255,255,255,0.02)', opacity: achieved ? 1 : 0.5 }}>
                <span style={{ fontSize: 20 }}>{r.icon}</span>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 13, color: '#e8f4ff', fontWeight: 600 }}>Day {r.day}</div>
