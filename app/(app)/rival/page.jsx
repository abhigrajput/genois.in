...
export default function RivalPage() {
  ...
  if (data?.isTopRanked) return (
    <div style={{ maxWidth: 600, margin: '0 auto', fontFamily: 'Outfit,sans-serif', textAlign: 'center', paddingTop: 60 }}>
      ...
      <h1 style={{ fontFamily: 'Syne,sans-serif', fontSize: 28, fontWeight: 800, color: '#EF9F27', marginBottom: 12 }}>
        You are #1
      </h1>
  ...
  return (
    <div style={{ maxWidth: 680, margin: '0 auto', fontFamily: 'Outfit,sans-serif' }}>
      ...
      <div style={{ background: '#070f1f', border: '1px solid rgba(255,45,120,0.2)', borderRadius: 16, padding: 24, marginBottom: 20, position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ fontFamily: 'JetBrains Mono,monospace', fontSize: 10, color: '#ff2d78', letterSpacing: 2, marginBottom: 20 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: 16, alignItems: 'center', marginBottom: 20 }}>
          ...
          <div style={{ textAlign: 'center', padding: 16, background: 'rgba(0,240,255,0.04)', borderRadius: 12, border: '1px solid rgba(0,240,255,0.15)' }}>
            <div style={{ fontFamily: 'JetBrains Mono,monospace', fontSize: 9, color: '#5a7a9a', letterSpacing: 1, marginBottom: 8 }}>YOU</div>
            <div style={{ fontFamily: 'Syne,sans-serif', fontSize: 28, fontWeight: 800, color: '#00f0ff', marginBottom: 4 }}>{data?.myScore}</div>
            <div style={{ fontSize: 11, color: '#5a7a9a', fontFamily: 'JetBrains Mono,monospace' }}>Rank #{data?.myRank}</div>
          </div>

          {/* VS */}
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontFamily: 'Syne,sans-serif', fontSize: 20, fontWeight: 800, color: '#ff2d78' }}>VS</div>
            <div style={{ fontSize: 10, color: '#3a4a5a', fontFamily: 'JetBrains Mono,monospace', marginTop: 4 }}>+{e.scoreDiff} pts behind</div>
          </div>
          ...
          <div style={{ textAlign: 'center', padding: 16, background: 'rgba(255,45,120,0.04)', borderRadius: 12, border: '1px solid rgba(255,45,120,0.15)' }}>
            <div style={{ fontFamily: 'JetBrains Mono,monospace', fontSize: 9, color: '#5a7a9a', letterSpacing: 1, marginBottom: 8 }}>RIVAL</div>
            <div style={{ fontFamily: 'Syne,sans-serif', fontSize: 28, fontWeight: 800, color: '#ff2d78', marginBottom: 4 }}>{e.score}</div>
            <div style={{ fontSize: 11, color: '#5a7a9a', fontFamily: 'JetBrains Mono,monospace' }}>Rank #{e.rank}</div>
          </div>
        </div>
        ...
        <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 10, padding: '12px 16px', textAlign: 'center' }}>
          <div style={{ fontSize: 14, color: '#e8f4ff', fontWeight: 600, marginBottom: 4 }}>
            You need <span style={{ color: '#ff2d78', fontFamily: 'Syne,sans-serif', fontWeight: 800 }}>{e.scoreDiff} more points</span> to overtake {e.name}
          </div>
      ...
      <div style={{ background: '#070f1f', border: `1px solid ${enemyColor}15`, borderRadius: 16, padding: 24, marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
          <div>
            <div style={{ fontFamily: 'Syne,sans-serif', fontSize: 20, fontWeight: 800, color: '#e8f4ff', marginBottom: 4 }}>
              {e.name}
            </div>
            <div style={{ fontSize: 13, color: '#5a7a9a', marginBottom: 8 }}>{e.college}</div>
            <div style={{ display: 'inline-block', padding: '3px 10px', borderRadius: 20, background: `${enemyColor}12`, border: `1px solid ${enemyColor}25`, color: enemyColor, fontSize: 11, fontFamily: 'JetBrains Mono,monospace' }}>
              {e.domain?.toUpperCase()}
            </div>
          </div>
          <div style={{ textAlign: 'right' }}>
            {e.activeToday ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: '#1D9E75', fontSize: 12, fontFamily: 'JetBrains Mono,monospace' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(120px,1fr))', gap: 10 }}>
          {[
            ...
            { label: 'Day', value: e.currentDay, color: '#7b5cff' },
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {[
            ...
            <div key={i} style={{ display: 'flex', gap: 14, padding: '14px', background: 'rgba(255,255,255,0.02)', borderRadius: 10, border: `1px solid ${s.color}12` }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontFamily: 'Syne,sans-serif', fontSize: 14, fontWeight: 700, color: s.color, marginBottom: 4 }}>{s.title}</div>
                <div style={{ fontSize: 13, color: '#5a7a9a', lineHeight: 1.6 }}>{s.desc}</div>
                {s.action && (
                  <button onClick={s.action} style={{ marginTop: 8, padding: '6px 14px', borderRadius: 8, border: `1px solid ${s.color}40`, background: `${s.color}10`, color: s.color, cursor: 'pointer', fontSize: 12, fontFamily: 'Syne,sans-serif', fontWeight: 600 }}>
                    {s.actionLabel}
                  </button>
                )}
              </div>
      ...
      <div style={{ background: 'linear-gradient(135deg,rgba(255,45,120,0.06),rgba(239,159,39,0.03))', border: '1px solid rgba(255,45,120,0.15)', borderRadius: 14, padding: 24, textAlign: 'center' }}>
        ...
        <button onClick={() => router.push('/roadmap')} style={{ padding: '13px 32px', borderRadius: 12, border: 'none', cursor: 'pointer', background: 'linear-gradient(135deg,#ff2d78,#EF9F27)', color: '#fff', fontFamily: 'Syne,sans-serif', fontSize: 15, fontWeight: 700, boxShadow: '0 0 20px rgba(255,45,120,0.25)' }}>
          Start Today&apos;s Tasks →
        </button>
      </div>
    </div>
