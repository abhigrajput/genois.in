    ...
    <div style={{ minHeight: '100vh', background: '#020812', color: '#e8f4ff', fontFamily: 'Outfit,sans-serif', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      ...
      <div style={{ width: '100%', maxWidth: 540, position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ background: '#070f1f', border: '1px solid rgba(0,240,255,0.12)', borderRadius: 14, padding: 28 }}>
          <div style={{ fontFamily: 'JetBrains Mono,monospace', fontSize: 10, color: '#00f0ff', letterSpacing: 2, marginBottom: 20 }}>COMPANY DETAILS</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {[
              ...
              <div key={f.key}>
                ...
                <input type={f.type} value={form[f.key]} onChange={e => setForm(p => ({ ...p, [f.key]: e.target.value }))} placeholder={f.placeholder} style={{ width: '100%', padding: '11px 14px', borderRadius: 10, border: '1px solid rgba(0,240,255,0.15)', background: 'rgba(255,255,255,0.03)', color: '#e8f4ff', fontSize: 14, outline: 'none', boxSizing: 'border-box' }} />
              </div>
            ))}

            <div>
              <div style={{ fontSize: 11, color: '#5a7a9a', fontFamily: 'JetBrains Mono,monospace', letterSpacing: 1, marginBottom: 6 }}>COMPANY SIZE</div>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {SIZES.map(s => (
                  <button key={s} onClick={() => setForm(p => ({ ...p, size: s }))} style={{ padding: '7px 14px', borderRadius: 20, border: `1px solid ${form.size === s ? 'rgba(0,240,255,0.4)' : 'rgba(255,255,255,0.08)'}`, background: form.size === s ? 'rgba(0,240,255,0.08)' : 'transparent', color: form.size === s ? '#00f0ff' : '#5a7a9a', cursor: 'pointer', fontSize: 12 }}>
                    {s}
                  </button>
                ))}
              </div>
            ...
            <div>
              <div style={{ fontSize: 11, color: '#5a7a9a', fontFamily: 'JetBrains Mono,monospace', letterSpacing: 1, marginBottom: 6 }}>HIRING DOMAIN</div>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {DOMAINS.map(d => (
                  <button key={d} onClick={() => setForm(p => ({ ...p, domainFocus: d }))} style={{ padding: '7px 14px', borderRadius: 20, border: `1px solid ${form.domainFocus === d ? 'rgba(0,240,255,0.4)' : 'rgba(255,255,255,0.08)'}`, background: form.domainFocus === d ? 'rgba(0,240,255,0.08)' : 'transparent', color: form.domainFocus === d ? '#00f0ff' : '#5a7a9a', cursor: 'pointer', fontSize: 12 }}>
                    {d}
                  </button>
                ))}
              </div>
          ...
          <button onClick={signup} disabled={loading} style={{ width: '100%', padding: '14px', borderRadius: 12, border: 'none', cursor: 'pointer', background: loading ? 'rgba(0,240,255,0.2)' : 'linear-gradient(135deg,#00f0ff,#7b5cff)', color: '#020812', fontFamily: 'Syne,sans-serif', fontSize: 15, fontWeight: 700, marginTop: 20, boxShadow: '0 0 24px rgba(0,240,255,0.2)' }}>
            {loading ? 'Creating account...' : 'Create Company Account — Free →'}
          </button>

          <p style={{ textAlign: 'center', color: '#3a4a5a', fontSize: 12, fontFamily: 'JetBrains Mono,monospace', marginTop: 10 }}>
