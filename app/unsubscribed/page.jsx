...
export default async function UnsubscribedPage({ searchParams }) {
  ...
  return (
    <div style={{ maxWidth: 640, margin: '80px auto', padding: '0 20px', fontFamily: 'var(--font-body)', color: '#e2e8f0' }}>
      <div style={{ fontSize: 24, fontWeight: 800, marginBottom: 8 }}>
        <span style={{ color: '#00d9a3' }}>GEN</span><span>OIS</span>
      </div>
      <div style={{ height: 2, background: `linear-gradient(90deg, ${state.tone}, transparent)`, marginBottom: 32 }} />

      <h1 style={{ fontFamily: 'var(--font-heading)', fontSize: 26, fontWeight: 800, color: state.tone, marginBottom: 16 }}>
        {state.heading}
      </h1>
      <p style={{ color: '#cbd5e1', lineHeight: 1.8, fontSize: 15, marginBottom: 20 }}>{state.body}</p>
      <p style={{ color: '#94a3b8', lineHeight: 1.8, fontSize: 13, marginBottom: 32 }}>{state.note}</p>

      {undo && (
        <p style={{ marginBottom: 24 }}>
          <a href={undo.href} style={{ color: '#5a7a9a', fontSize: 13, textDecoration: 'underline' }}>{undo.label}</a>
        </p>
      )}
      ...
      <a
        ...
        style={{
          ...
          background: 'linear-gradient(135deg,#00d9a3,#ff6b4a)', color: '#0a0a0f', textDecoration: 'none',
        }}
      >
        Back to GENOIS
      </a>
